...
```yml
flink-master:
   image: bde2020/flink-master:1.6.0-hadoop2.8
...
#     - "constraint:node==<yourmasternode>"
 ...
 flink-worker:
   image: bde2020/flink-worker:1.6.0-hadoop2.8
...
To start a Flink master:
    ...
    docker run --name flink-master --net flink-net -e ENABLE_INIT_DAEMON=false -d bde2020/flink-master:1.6.0-hadoop2.8

## Flink Worker
To start a Flink worker:

    docker run --name flink-worker --net flink-net -e ENABLE_INIT_DAEMON=false -e FLINK_MASTER_PORT_6123_TCP_ADDR=flink-master -d bde2020/flink-worker:1.6.0-hadoop2.8
